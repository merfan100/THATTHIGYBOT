...
# اگر ادمین ست نشده بود دیفالت رو میذاره، اما بهتره تو رندر ست کنی
...
# توجه: این متغیر چون در حافظه رم هست، با هر بار ریستارت شدن سرور رندر، مقدارش ریست میشه
registration_status = {
    "esfahan": False,
    "tehran": False,
    "shiraz": False,
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ------------------------- تأیید / رد فیش‌ها -------------------------
        ...
        # parts[0] -> confirm
        # parts[1] -> user_id
        # parts[2] -> city_name (اختیاری اگر توی دکمه گذاشته باشی)
        
        user_id = int(parts[1])
        # city_name = parts[2] اگر نیاز بود
...
async def main():
    ...
    # تغییر مهم: جلوگیری از هندل کردن سیگنال توسط تلگرام چون فلاسک هندلش میکنه
    # یا استفاده از updater.start_polling() اگر از ورژن‌های قدیمی تر استفاده میکردی
    # اما در ورژن 20 به بالا run_polling استفاده میشه.
    # پارامتر stop_signals=None باعث میشه با هایپرکورن دعوا نکنه
    await app.updater.start_polling(allowed_updates=Update.ALL_TYPES)
    
    # اینجا یک حلقه بی نهایت ساده میذاریم که تسک تموم نشه
    # چون run_polling اگر با stop_signals=None باشه بلاک نمیکنه (بسته به ورژن)
    # ولی برای اطمینان بهتره از await future استفاده کنیم که در server.py هندل شده.
    # در واقع ما اینجا فقط polling رو استارت زدیم. و صبر میکنیم.
    
    # روش تمیزتر برای اینکه تسک بسته نشه:
